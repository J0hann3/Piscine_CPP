...
void _print()
{
	for (std::vector<int>::iterator it = test.begin(); it != test.end(); it = it + 1)
	{
		std::cout << *it << " ";
	}
	std::cout << std::endl;
...
void _print(RandomAccessIterator first, RandomAccessIterator last)
{
...
int main()
{
	test = {-1,-5,10,7,0,14,6,5,4,3,1, 0};
	merge_insertion_sort(test.begin(), test.end());
	_print();
}